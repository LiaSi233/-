# 从搭建到训练安全大模型
```markdown
> 基于 Qwen3-4B 基座模型，通过 QLoRA 技术微调的网络安全领域大语言模型，集成多智能体自动化渗透测试框架，实现威胁情报理解与攻防场景智能化。
...
本项目针对传统网络安全防御体系在应对海量告警与复杂攻击时存在的"研判滞后、专家资源匮乏"等痛点，以及通用大模型在安全垂直领域专业度不足的问题，提出一种基于大模型的全流程智能化攻防应用框架。

### 核心工作

1. **模型微调**：基于 LLaMA-Factory 微调架构，构建标准化的 ShareGPT 安全指令数据集，对 Qwen3-4B 基座模型进行深度监督微调（SFT），显著提升模型对威胁情报的语义理解与逻辑推理能力。

2. **边缘部署优化**：引入 GGUF 格式转换与 4-bit 量化技术，通过内存映射机制解决大模型在边缘设备的部署瓶颈，实现了推理能效与响应速度的最佳平衡。

3. **自动化攻防应用**：结合 Strix 多智能体（Agent）框架在靶场环境中开展自动化渗透测试实验，高效完成资产侦察、漏洞定位及认证绕过等任务，较传统人工流程缩短 90% 的作业周期。

### 关键指标

| 指标 | 基座模型 | 微调后模型 | 提升 |
|------|---------|-----------|------|
| BLEU-4 | 0.124 | 0.287 | +131% |
| ROUGE-L | 0.231 | 0.445 | +93% |
| 响应延迟 | 2.3s | 1.1s | -52% |
| 模型体积 | 7.8GB | 2.1GB | -73% |
...
随着"万物互联"加速落地，全球网络流量年复合增长率已突破 25%。2024 年中国数字经济规模已超过 GDP 的 40%（中国信通院，2024）。然而，网络攻击面呈指数级扩张：

- 2023 年国家级 APT 组织平均潜伏时间高达 112 天
- 勒索软件即服务（RaaS）交易规模达 141 亿美元（ENISA，2024）
- 传统"防火墙+IDS+SIEM"线性堆叠架构在检测精度、响应时延与运维成本三方面陷入"三元悖论"

### 上海实践案例

上海三年投入 47 亿元打造三级网络安全态势感知平台，接入 18 万台设备、覆盖 3.2 万数字资产，日均处理 4.7TB 日志、2.3 亿条告警。然而，平台面临严峻挑战：

- **有效告警率低**：仅 0.8% 的告警具有实际价值
- **响应速度慢**：人工研判需 3-5 小时，远落后于攻击速度（勒索软件仅需 1.2 小时）
- **人力缺口大**：安全人员仅 0.47 万，人机比高达 1:142

### 研究价值

党的二十大报告明确提出："要完善国家安全风险监测预警体系，构建全域联动、立体高效的国家安全防护体系"。2025 年 1 月 20 日，DeepSeek-R1 模型正式发布，以 7200 亿参数与多模态融合能力为标志，生成式人工智能正式迈入"千亿级"新纪元，为本项目提供了可落地的技术底座。
